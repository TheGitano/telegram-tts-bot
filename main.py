...
def analyze_image_with_claude(image_bytes, image_mime_type="image/jpeg", mode="analyze"):
    ...
    try:
        ...
        # Modelos con soporte de visión (imagen) - ordenados por preferencia
        # Formato: (nombre_modelo, api_version)
        models_to_try = [
                ...
                if resp.status_code == 200:
                    ...
                    if candidates:
                        ...
                        if text and len(text.strip()) > 3:
                            logger.info(f"✅ Gemini ({model_name}) respondió OK - {len(text)} chars")
                            return text.strip()
                    logger.warning(f"Modelo {model_name} respondió 200 pero sin texto útil")
                else:
                    try:
                    ...
                    logger.error(f"Modelo {model_name} falló ({resp.status_code}): {error_msg}")

...
# ============================================================
# FUNCIONES DE IMAGEN CON GOOGLE VISION + GEMINI
# ============================================================

def extract_text_with_vision(image_bytes):
    """Extrae texto de imagen usando Google Cloud Vision API."""
    try:
        import requests
        ...
        if not creds_json:
            logger.error("No se encontró GOOGLE_APPLICATION_CREDENTIALS_JSON")
            return None
...
def analyze_image_with_claude(image_bytes, image_mime_type="image/jpeg", mode="analyze"):
    """
    Analiza una imagen usando Google Vision (OCR) + Gemini (análisis).
    ...
    try:
        ...
        if not extracted_text:
            # Si Vision no detectó texto, usar Gemini con la imagen directamente (base64)
            logger.info("Vision no detectó texto, usando Gemini con imagen...")
            image_b64 = base64.standard_b64encode(image_bytes).decode("utf-8")
            ...
            if mode == "extract":
                prompt = (
                    ...
                    "No agregues interpretaciones. Solo el texto exacto."
                )
            else:
                prompt = (
                    "Eres un experto multidisciplinario legal, financiero y de asesoría. "
                    "Analiza la imagen:\n\n"
                    "1. 📋 EXTRACCIÓN DE TEXTO: Transcribe todo el texto visible.\n\n"
                    "2. 📊 ANÁLISIS DETALLADO: Explica qué significa el contenido.\n\n"
                    "3. ⚠️ SITUACIÓN ACTUAL: Describe urgencias y riesgos.\n\n"
                    "4. ✅ CONSEJOS: Recomendaciones concretas como abogado/asesor.\n\n"
                    "5. 📌 CONCLUSIÓN: Puntos más importantes.\n\n"
        ...
        prompt = (
            f"Eres un experto multidisciplinario legal, financiero y de asesoría. "
            f"Analiza el siguiente texto extraído de un documento:\n\n"
            f"---TEXTO---\n{extracted_text}\n---FIN---\n\n"
            f"1. 📋 TEXTO EXTRAÍDO: Muestra el texto original.\n\n"
            f"2. 📊 ANÁLISIS DETALLADO: Explica el contenido, destaca fechas, montos y datos clave.\n\n"
            f"3. ⚠️ SITUACIÓN ACTUAL: Urgencias y riesgos críticos.\n\n"
            f"4. ✅ CONSEJOS Y RECOMENDACIONES: Pasos concretos como abogado/asesor.\n\n"
            f"5. 📌 CONCLUSIÓN: Puntos más urgentes.\n\n"
            f"Responde en el idioma del texto. Usa emojis para organizar."
        )
